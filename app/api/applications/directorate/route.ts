...
import { directorateApplicationSchema } from "@/lib/application-schema";
import { saveDirectorateApplication } from "@/lib/application-store";
import { opportunities } from "@/lib/content";

export const runtime = "nodejs";
// Turnstile verify + dup checks + two file uploads to Supabase can add up on
// a slow mobile connection; give it real headroom above the 10s default.
...
export async function POST(request: Request) {
  const applicationsOpen = opportunities.some((item) => item.id === "directorate" && item.status === "open");
  if (!applicationsOpen) {
    return NextResponse.json({ message: "Directorate applications are closed." }, { status: 410 });
  }

  if (rateLimited(request)) return NextResponse.json({ message: "Too many attempts. Please try again shortly." }, { status: 429 });

  const submission = await parseApplicationSubmission(request, { withCv: true });
  if (!submission.ok) {
    return NextResponse.json({ message: submission.message, issues: submission.issues }, { status: 400 });
  }

  const parsed = directorateApplicationSchema.safeParse(submission.input);
  if (!parsed.success) {
  ...
  try {
    const result = await saveDirectorateApplication(parsed.data, submission.photo, submission.cv!);
    ...
    if (error instanceof Error && error.message === "DUPLICATE_APPLICATION") {
      return NextResponse.json({ message: "A Directorate application using this email, WhatsApp number or CNIC has already been received." }, { status: 409 });
    }
    console.error("Directorate application error", error);
