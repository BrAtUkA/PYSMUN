...
import { delegateApplicationSchema } from "@/lib/application-schema";
import { saveDelegateApplication } from "@/lib/application-store";
...
export async function POST(request: Request) {
  ...
  const parsed = delegateApplicationSchema.safeParse(submission.input);
  if (!parsed.success) {
  ...
  const fee = currentDelegateFee();

  try {
    const result = await saveDelegateApplication(
